...
add address=172.16.255.4/31 interface=eBGP_LINK_AS65001_2 network=\
    172.16.255.4
/ip dhcp-client
add check-gateway=ping interface=sfp-sfpplus12
...
add action=accept chain=forward comment=\
    ...
    established,related
add action=accept chain=input comment="allowing incoming BGP sessions" \
    in-interface-list=ZONE_BGP_AS65001 port=179 protocol=tcp
...
add action=accept chain=forward dst-address=172.16.255.6/31 \
...
/ip firewall nat
add action=masquerade chain=srcnat out-interface-list=ZONE-WAN to-addresses=\
    10.0.0.247
/ip route
